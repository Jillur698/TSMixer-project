...
class TSFDataLoader:
    ...
    def __init__(self, data, batch_size, seq_len, pred_len, feature_type, target='OT'):
        ...
        logger.info(f"Initializing data loader for {data}")
        self._read_data()
    ...
    def _read_data(self):
        ...
        if not os.path.isfile(cache_filepath):
            logger.info(f"Copying data from {DATA_DIR} to {LOCAL_CACHE_DIR}")
            tf.io.gfile.copy(os.path.join(DATA_DIR, file_name), cache_filepath, overwrite=True)

        logger.info(f"Loading data from {cache_filepath}")
        ...
        n = len(df)
        logger.info(f"Total records in dataset: {n}")
        ...
        logger.info(f"Number of features: {self.n_feature}")
    ...
    def _make_dataset(self, data, shuffle=True):
        ...
        logger.info(f"Dataset created with {len(data)} records, shuffle={shuffle}")
        return ds
